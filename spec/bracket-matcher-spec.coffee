describe "bracket matching", ->
  ...
  beforeEach ->
    atom.config.set 'bracket-matcher-highlight.autocompleteBrackets', true

    waitsForPromise ->
      atom.packages.activatePackage('bracket-matcher-highlight')
  ...
  describe "matching bracket highlighting", ->
    expectNoHighlights = ->
      decorations = editor.getHighlightDecorations().filter (decoration) -> decoration.properties.class is 'bracket-matcher-highlight'
  ...
  describe "when bracket-matcher-highlight:go-to-matching-bracket is triggered", ->
    describe "when the cursor is before the starting pair", ->
      it "moves the cursor to after the ending pair", ->
        editor.moveToEndOfLine()
        editor.moveLeft()
        atom.commands.dispatch(editorElement, "bracket-matcher-highlight:go-to-matching-bracket")
  ...
  describe "when bracket-matcher-highlight:go-to-enclosing-bracket is triggered", ->
    describe "when within a `{}` pair", ->
      it "moves the cursor to before the enclosing brace", ->
        editor.setCursorBufferPosition([11, 2])
        atom.commands.dispatch(editorElement, "bracket-matcher-highlight:go-to-enclosing-bracket")
        expect(editor.getCursorBufferPosition()).toEqual [0, 28]
  ...
  describe "when bracket-match:select-inside-brackets is triggered", ->
    describe "when the cursor on the left side of a bracket", ->
      it "selects the text inside the brackets", ->
        ...
        atom.commands.dispatch(editorElement, "bracket-matcher-highlight:select-inside-brackets")
  ...
  describe "when bracket-matcher-highlight:remove-matching-brackets is triggered", ->
    describe "when the cursor is not in front of any pair", ->
      it "performs a regular backspace action", ->
        editor.setCursorBufferPosition([0, 1])
        atom.commands.dispatch(editorElement, "bracket-matcher-highlight:remove-matching-brackets")
  ...
  describe "matching bracket deletion", ->
    ...
    describe "when selection is not a matching pair of brackets", ->
      it "does not change the text", ->
        ...
        editor.selectAll()
        atom.commands.dispatch(editorElement, "bracket-matcher-highlight:remove-brackets-from-selection")
  ...
  describe "matching bracket insertion", ->
    ...
    describe "when autocompleteBrackets configuration is disabled globally", ->
      it "does not insert a matching bracket", ->
        atom.config.set 'bracket-matcher-highlight.autocompleteBrackets', false
        editor.buffer.setText("}")
        editor.setCursorBufferPosition([0, 0])
        editor.insertText '{'
    ...
    describe "when autocompleteBrackets configuration is disabled in scope", ->
      it "does not insert a matching bracket", ->
        atom.config.set 'bracket-matcher-highlight.autocompleteBrackets', true
        atom.config.set 'bracket-matcher-highlight.autocompleteBrackets', false, scopeSelector: '.source.js'
        editor.buffer.setText("}")
        editor.setCursorBufferPosition([0, 0])
        editor.insertText '{'
        expect(buffer.lineForRow(0)).toBe "{}"
    ...
    describe "when autocompleteCharacters configuration is set globally", ->
      it "inserts a matching angle bracket", ->
        atom.config.set 'bracket-matcher-highlight.autocompleteCharacters', ['<>']
        editor.setCursorBufferPosition([0, 0])
        editor.insertText '<'
        expect(buffer.lineForRow(0)).toBe "<>"
        expect(editor.getCursorBufferPosition()).toEqual([0, 1])
    ...
    describe "when autocompleteCharacters configuration is set in scope", ->
      it "inserts a matching angle bracket", ->
        atom.config.set 'bracket-matcher-highlight.autocompleteCharacters', ['<>'], scopeSelector: '.source.js'
        editor.setCursorBufferPosition([0, 0])
        editor.insertText '<'
        expect(buffer.lineForRow(0)).toBe "<>"
        expect(editor.getCursorBufferPosition()).toEqual([0, 1])
    ...
    describe "when there is text selected on a single line", ->
      ...
      describe "when the bracket-matcher-highlight.wrapSelectionsInBrackets is falsy globally", ->
        it "does not wrap the selection in brackets", ->
          atom.config.set('bracket-matcher-highlight.wrapSelectionsInBrackets', false)
          editor.setText 'text'
          editor.moveToBottom()
          editor.selectToTop()
          editor.selectAll()
      ...
      describe "when the bracket-matcher-highlight.wrapSelectionsInBrackets is falsy in scope", ->
        it "does not wrap the selection in brackets", ->
          atom.config.set('bracket-matcher-highlight.wrapSelectionsInBrackets', true)
          atom.config.set('bracket-matcher-highlight.wrapSelectionsInBrackets', false, scopeSelector: '.source.js')
          editor.setText 'text'
          editor.moveToBottom()
          editor.selectToTop()
          editor.selectAll()
  ...
  describe 'bracket-matcher-highlight:close-tag', ->
    beforeEach ->
      waitsForPromise ->
    ...
    it 'closes the first unclosed tag', ->
      ...
      atom.commands.dispatch(editorElement, 'bracket-matcher-highlight:close-tag')
