...
class BracketMatcher
  ...
  constructor: (@editor, editorElement) ->
    ...
    @subscriptions.add atom.commands.add editorElement, 'bracket-matcher-highlight:remove-brackets-from-selection', (event) =>
  ...
  insertText: (text, options) =>
    ...
    @toggleQuotes(@getScopedSetting('bracket-matcher-highlight.autocompleteSmartQuotes'))
    ...
    if text is '#' and @isCursorOnInterpolatedString()
      autoCompleteOpeningBracket = @getScopedSetting('bracket-matcher-highlight.autocompleteBrackets') and not hasEscapeSequenceBeforeCursor
      text += '{'
      pair = '}'
    else
      autoCompleteOpeningBracket = @getScopedSetting('bracket-matcher-highlight.autocompleteBrackets') and @isOpeningBracket(text) and not hasWordAfterCursor and not (@isQuote(text) and (hasWordBeforeCursor or hasQuoteBeforeCursor)) and not hasEscapeSequenceBeforeCursor
  ...
  backspace: =>
    ...
    hasEscapeSequenceBeforeCursor = previousCharacters.match(/\\/g)?.length >= 1 # To guard against the "\\" sequence
    if (@pairedCharacters[previousCharacter] is nextCharacter) and not hasEscapeSequenceBeforeCursor and @getScopedSetting('bracket-matcher-highlight.autocompleteBrackets')
      @editor.transact =>
        @editor.moveLeft()
        @editor.delete()
        @editor.delete()
  ...
  wrapSelectionInBrackets: (bracket) ->
    return false unless @getScopedSetting('bracket-matcher-highlight.wrapSelectionsInBrackets')
